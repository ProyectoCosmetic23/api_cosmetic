...
async function createEmployee(req, res) {
  ...
  if (!/^[A-Za-z0-9\s~áéíóúÁÉÍÓÚñÑ]+$/.test(name_employee)) {
    return res.status(400).json({ error: 'El nombre debe contener letras, números, espacios y el símbolo "~" para la "ñ" con tilde' });
}
...
async function employeeChangeStatus(req, res) {
  const { id } = req.params;
  const { reason_anulate } = req.body; // Agrega esta línea para obtener la razón de la anulación
