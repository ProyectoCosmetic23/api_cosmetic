...
const getAllReturns = async (req, res) => {
  try {
    ...
    if (returns.length === 0) {
      return res.status(404).json({ message: "No hay devoluciones registradas" });
    }
    res.json(returns);
  } catch (error) {
...
const processReturn = async (req, res) => {
    ...
    try {
        ...
        if (damaged_product) {
            ...
            await Products.update({ quantity: newQuantity }, { where: { id_product: id_returned_product } });
            const registeredReturn = await register_return();
            res.json({"El stock del producto fue actualizado con éxito": registeredReturn});
        }
    } catch (err) {
        // Manejar errores
        console.error(err);
...
const createNewSaleAndCancelOldSale = async (req, res) => {
    ...
    try {
        ...
        if (isNaN(lastInvoiceNumber)) {
            return res.status(500).json({ error: 'No se pudo obtener el número de factura.' });
        }
        ...
        for (var product of products) {
            if (isNaN(product.product_price) || isNaN(product.quantity)) {
                return res.status(400).json({ error: 'Los productos deben tener precios y cantidades válidos.' });
            }
        ...
        res.json({ newSale: new_sale, saleDetail: sale_detail, totalSale: total_sale });
    } catch (err) {
