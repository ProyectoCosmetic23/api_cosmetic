...
async function createPay(req, res) {
  let { id_sale, id_order, id_client, total_payment } = req.body;
  ...
  try {
    ...
    if (id_sale != null && id_sale != undefined) {
      ...
      if (sale.payment_state === "Pagado") {
        return res
          .status(400)
          .json({ error: "Esta venta ya está pagada y no admite más pagos." });
      }
...
async function getPayClien(req, res) {
  ...
  try {
    ...
    if (payCliente.length === 0) {
      return res
        .status(404)
        .json({ message: "No hay pagos para este cliente" });
    }
    res.json(payCliente);
  } catch (error) {
    console.error("Error fetching payments:", error);
